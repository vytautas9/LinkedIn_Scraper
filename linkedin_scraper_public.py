# -----------------------------------------------------------
# Reads linkedin job offers from publicly available data.
# Saves the data into dataframe and saves as csv file.
#
# (C) 2023 Vytautas Kraujalis, Lithuania
...
# are we gonna use proxy (True) or no (False) to read linkedin data?
use_proxy = False
...
def open_job_list_page(job_position, job_location):
    ...
    driver.get('https://linkedin.com/jobs/--jobs')
    time.sleep(2)

    # Enter the job_position and job_location into search bars
    search_keywords = driver.find_elements(By.CSS_SELECTOR,
                                           'input[aria-controls="job-search-bar-keywords-typeahead-list"]')[0]
    search_keywords.clear()
    search_keywords.send_keys(job_position)

    search_location = driver.find_elements(By.CSS_SELECTOR,
                                           'input[aria-controls="job-search-bar-location-typeahead-list"]')[0]
    ...
    # deny cookies
    time.sleep(2)
    cookie_deny = driver.find_elements(By.CSS_SELECTOR, 'button[data-control-name="ga-cookie.consent.deny.v4"]')
    cookie_deny[0].click()


def get_number_of_available_offers():
    """
    Gets the maximum number of available offers at the current opened link
...
def get_linkedin_job_links(job_position, job_location, old_job_links):
    ...
    # Get the maximum number of available offers
    jobs_num = get_number_of_available_offers()
    print(f'There are {jobs_num} jobs to be read.')
    print(f'There should be approximately {int(jobs_num / 25)} pages if we consider each page has 25 jobs.')
    ...
    # With public data, there's no pages to browse, so we firstly need to scroll down to load all job offers
    # We create a while loop to browse all jobs.
    time.sleep(2)
    print(f'Starting the scrolling down to ensure all job offers are rendered on the screen.')
    i = 0
    while i <= int(jobs_num/25):
        # We keep scrolling down to the end of the view.
        driver.execute_script('window.scrollTo(0, document.body.scrollHeight);')

        print('\rCurrent page: ', i, 'Percentage at: ', '{:.2f}'.format((i / (int(jobs_num / 25))) * 100), '%', end='')
        i = i + 1
    ...
    try:
        ...
        jobs_list = jobs_block.find_elements(By.CSS_SELECTOR, '.base-search-card')

        for idx, job in enumerate(jobs_list, start=1):
            print('\rCurrently reading link: ', idx, 'Percentage at: ', '{:.2f}'.format((idx / jobs_num) * 100), '%',
                  end='')

            job_id = job.get_attribute('data-entity-urn')
            job_id = re.findall(r'\d+', job_id)[-1]
            ...
            if link not in job_links['job_link'] and job_id not in old_ids:
                ...
                job_links = pd.concat([job_links,
                                       pd.DataFrame(data, index=[number_of_unique_links-1])], ignore_index=True)
            else:
                pass
    ...
    for i in range(len(urls)):
        ...
        try:
            ...
            driver.find_element(By.CSS_SELECTOR,
                                'button[aria-label="Show more, visually expands previously read content above"]').click()
            time.sleep(2)
        except Exception:
        ...
        content = driver.find_element(By.CSS_SELECTOR, '.top-card-layout__entity-info')
        top_cards = content.find_elements(By.CLASS_NAME, "topcard__flavor-row")
        ...
        company_info = top_cards[0].find_elements(By.CLASS_NAME, 'topcard__flavor')

        # Company Name
        if len(company_info):
            company_names.append(company_info[0].text)
        else:
        ...
        # Company Location
        if len(company_info):
            company_locations.append(company_info[1].text)
        else:
            company_locations.append("")
            print(f'\t\t\tCompany location was not found...')

        # Post Date
        post_date = top_cards[1].find_elements(By.CSS_SELECTOR,
        ...
        job_description = driver.find_elements(By.CLASS_NAME, 'decorated-job-posting__details')[0]
        job_text = job_description.find_elements(By.CLASS_NAME, "show-more-less-html__markup")
        if len(job_text):
